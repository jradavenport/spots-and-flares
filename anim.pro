pro anim
  ...
  ;stsp_ver = '/Users/james/src/STSP/stsp_L'

  ; read the starspot VIZ outputs from STSP
  ;readcol, 'spot_v1_vis.txt', t, xx, yy, aa, bb, col, /silent
  readcol, 'spot_v2.in', inraw, f='(F)', /silent
  if inraw[0] eq 0 then $ ; if no planets
     nspots = inraw[8]
  readcol, 'spot_v2_lcout.txt', time, flux, f='(F,X,X,F)', /silent
  flux_orig = flux

  rad = inraw[n_elements(inraw) - (findgen(nspots)*3.+4.)]
  lat = inraw[n_elements(inraw) - (findgen(nspots)*3.+3.)]/!dpi*180.-90.
  lon = 360 - inraw[n_elements(inraw) - (findgen(nspots)*3.+2.)]/!dpi*180.-180.
  ...
  ; the blank light curve
  ;; time = findgen(nframes)
  ;; flux = fltarr(nframes)
  ...
  FOR i=0l,nframes-1 DO BEGIN
     ...
     if new_fl_p le flare_rate then begin
        ...
        flare_lon = -i + randomu(sss, 1)*90.-45.
  ...
  spawn,' ffmpeg -r 20 -i img/frame%05d.jpeg -pix_fmt yuv420p -qscale 1 test.mp4 '
